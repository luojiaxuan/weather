...
public class MapActivity extends AppCompatActivity implements LocationSource,AMapLocationListener {
    ...
    private String cityName = "";//定位当前位置
    @Override
    ...
    public void onLocationChanged(AMapLocation aMapLocation) {
        if (aMapLocation != null) {
            if (aMapLocation.getErrorCode() == 0) {
                ...
                //Toast.makeText(getApplicationContext(),"ErrorCode:"+aMapLocation.getErrorCode()+"\nErrorInfo:"+aMapLocation.getErrorInfo(),Toast.LENGTH_LONG).show();
                Toast.makeText(getApplicationContext(), "定位被禁用，请到设置授予定位权限", Toast.LENGTH_LONG).show();
            }
        }
    }
